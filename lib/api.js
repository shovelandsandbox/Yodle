...
Api.prototype.getOptions = function(url, port) {
  ...
  if(this.config.token) {
    config.headers['x-access-token'] = this.config.token;
  }

  config.protocol = this.config.protocol;
...
Api.prototype.call = function(data, path, method) {
  return new Promise(function (_resolve, _reject) {
    ...
    var options = this.getOptions(this.config.server, this.config.port);
